...
aucs_ergo_mcpas <- array(0,dim=c(1,length(tcr_list)))
...
for (TCR in setdiff(1:11,c(8,9))){
  ...
  output_data <- read.csv(paste0("C:/Users/amita/Downloads/epitope_analysis/",
             ...
             "misc_methods/ergo//ergo_output_mcpas.csv"))
  
  peptide_rank <- as.double(output_data$Score[match(peptide_list,output_data$Peptide)]) #find peptide ranks
  ...
  aucs_ergo_mcpas[1,TCR] <- mean(AUROCs)
}

save(aucs_ergo_mcpas,file=paste0("C:/Users/amita/Downloads/epitope_analysis/",
                      "antigen-availability-master/antigen-availability-master/",
                      "TCR-crossreactivity/final_codes/local_outputs/",
                      "misc_methods/ergo/ergo_mcpas_outputs.rda"))