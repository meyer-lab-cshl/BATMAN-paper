...
tcr_list <- c(6,12,52,55,58,59,60,62,64,65)
...
# Separate peptides into training folds
...
AUROCs<-array(0,dim=c(3,nfold))
for (i in 1:(nfold)){
  ROC<-multiclass.roc(peptide_binding_category[training_folds==i],index_peptide_distances[training_folds==i,1],direction="<")
  AUROCs[,i]<-c(auc(ROC$rocs[[1]]),auc(ROC$rocs[[2]]),auc(ROC$rocs[[3]]))
}

print(rowMeans(AUROCs))
