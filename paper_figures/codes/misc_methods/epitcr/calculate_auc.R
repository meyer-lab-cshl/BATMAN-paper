...
aucs_epitcr <- array(0,dim=c(1,length(tcr_list)))
...
for (TCR in setdiff(1:11,c(8,9))){
  ...
  output_data <- read.csv(paste0("C:/Users/amita/Downloads/epitope_analysis/",
             ...
             "misc_methods/epitcr//output.csv"))
  
  peptide_rank <- as.double(output_data$predict_proba[match(peptide_list,output_data$epitope)]) #find peptide ranks
  ...
  aucs_epitcr[1,TCR] <- mean(AUROCs)
}

save(aucs_epitcr,file=paste0("C:/Users/amita/Downloads/epitope_analysis/",
                      "antigen-availability-master/antigen-availability-master/",
                      "TCR-crossreactivity/final_codes/local_outputs/",
                      "misc_methods/epitcr/epitcr_outputs.rda"))