...
aucs_prime <- array(0,dim=c(1,length(tcr_list)))
...
for (TCR in 1:11){
  ...
  peptide_rank <- as.double(output_data$V2[match(peptide_list,output_data$V1)]) #find peptide ranks
  ...
  aucs_prime[1,TCR] <- mean(AUROCs)
}

save(aucs_prime,file=paste0("C:/Users/amita/Downloads/epitope_analysis/",
                      "antigen-availability-master/antigen-availability-master/",
                      "TCR-crossreactivity/final_codes/local_outputs/",
                      "misc_methods/prime/prime_outputs.rda"))