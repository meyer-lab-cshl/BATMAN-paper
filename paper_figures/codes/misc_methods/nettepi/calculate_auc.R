...
aucs_nettepi <- array(0,dim=c(1,length(tcr_list)))
...
for (TCR in 1:11){
  ...
  file <- paste0("C:/Users/amita/Downloads/epitope_analysis/",
                 ...
                 "misc_methods/nettepi//NetTepi_out_",mhc_list[TCR],".xls")
  if (file.exists(file)){
  output_data <- read.table(file)
  
  
  peptide_score <- as.double(output_data$V8[match(peptide_list,output_data$V2)]) #find peptide ranks
  ...
  aucs_nettepi[1,TCR] <- mean(AUROCs)
  }
}

save(aucs_nettepi,file=paste0("C:/Users/amita/Downloads/epitope_analysis/",
                      "antigen-availability-master/antigen-availability-master/",
                      "TCR-crossreactivity/final_codes/local_outputs/",
                      "misc_methods/nettepi/nettepi_outputs.rda"))