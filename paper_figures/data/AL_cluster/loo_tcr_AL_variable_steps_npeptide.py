...
npeptide_al = int(sys.argv[3]) # number of peptides per step per site for AL
npeptide_rand = npeptide - npeptide_al
...
#Array to save data
n_al_step = int(90/(9*npeptide)) # max number of AL steps
...
for left_out_TCR_index in np.arange(len(TCR_list)): # loop over TCR index for test TCR
    ...
    train_data.to_csv(''.join(['tmp_train_data_',str(npeptide),str(npeptide_al),str(rand_seed),'.csv']), index = False)
    ...
    inferred_weights_pan,_,inferred_matrix_pan = train(''.join(['tmp_train_data_',str(npeptide),str(npeptide_al),str(rand_seed),'.csv']),
                                                                 'full',
                                                                 'blosum100',
                                                                 steps = 80000,
                                                                 seed = 100+rand_seed)
    ...
    # Set flat weight prior sd for first cycle
    ...
    for step in np.arange(n_al_step):
        ...
        # For every position, get mutants
        train_indices = np.array([])  #initialize
        ...
        for mutation in np.arange(peptide_length): # peptide positions to sample
            ...
            # Pick judiciously npeptide_al peptides
            if w_ranks[0,mutation]>=5:             
                distance_index =  np.argsort(abs(distances_position-distance1))[0:npeptide_al]
                
                train_indices_added = mutation_position[
                mutation_position.position==mutation].index[distance_index]                                         

            else:                
                distance_index =  np.argsort(abs(distances_position-distance2))[0:npeptide_al]
            ...
            #Pick random peptides
            if npeptide_rand>0:
                # indices available to pick from
                remaining_indices = np.setdiff1d(mutation_position[
                mutation_position.position==mutation].index,train_indices)
...
roc_auc_active.to_csv(''.join(['data/auc_npeptide_',str(npeptide),'_al_',str(npeptide_al),'_',str(rand_seed),'.csv']))
