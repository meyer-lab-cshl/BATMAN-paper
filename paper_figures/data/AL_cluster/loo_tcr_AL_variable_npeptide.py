...
for left_out_TCR_index in np.arange(len(TCR_list)): # loop over TCR index for test TCR
    ...
    train_data.to_csv(''.join(['tmp_train_data_',
                               str(npeptide),'_',str(nstep),
                               '_',str(rand_seed),'.csv']), index = False)
    ...
    inferred_weights_pan,_,inferred_matrix_pan = train(''.join(['tmp_train_data_',
                               str(npeptide),'_',str(nstep),
                               '_',str(rand_seed),'.csv']),
                                                       'full',
                                                       'blosum100',
...
roc_auc_active.to_csv(''.join(['data/auc_npeptide_',str(npeptide),'_nstep_',str(nstep),'_',str(rand_seed),'.csv']))
