...
### discard last few steps
plot_data <- plot_data[!plot_data$method %in% c("54","63","72", "81", "90"),]
...
p <- p + geom_boxplot(aes(color=factor(type)),
                      outlier.colour = NA,
                      show.legend = TRUE, alpha=1,lwd=0.6,
                      position=position_dodge(0.8)) +
  scale_color_manual(values=c('#7570b3','#984ea3','#4daf4a')) +
...
ggsave(plot=p, "../figures/fig2/auc_al.pdf",width=15, height=10)
