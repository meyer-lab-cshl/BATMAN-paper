...
_,inferred_matrix_full_tcr_specific = train('batman_train_data.csv','full',
                           'blosum100', steps = 80000, seed = 100)
_,inferred_matrix_symm_tcr_specific = train('batman_train_data.csv','symm',
                           'blosum100', steps = 80000, seed = 100)
...
# Train BATMAN to infer TCR-specific weights
_,inferred_matrix_full_pan_tcr = train('batman_train_data.csv','full',
                           'blosum100', steps = 80000, seed = 100)
_,inferred_matrix_symm_pan_tcr = train('batman_train_data.csv','symm',
                           'blosum100', steps = 80000, seed = 100)
...
full_specific = inferred_matrix_full_tcr_specific.loc[AA_list,AA_list]
symm_specific = inferred_matrix_symm_tcr_specific.loc[AA_list,AA_list]

full_pan = inferred_matrix_full_pan_tcr.loc[AA_list,AA_list]
symm_pan = inferred_matrix_symm_pan_tcr.loc[AA_list,AA_list]
...
plt.subplot(2, 2, 1)
sns.heatmap((full_specific/2.984)/blosum,cmap="RdBu",
...
plt.xlabel('To AA');
plt.ylabel('From AA');
plt.title('Full matrix for TCR-specific weights');

plt.subplot(2, 2, 2)
sns.heatmap((full_pan/2.279)/blosum,cmap="RdBu",
            # divided the pooled mean of AA matrix So that ratio with blosum=1 
            yticklabels=AA_list,
            xticklabels=AA_list,center=1,vmin=0,vmax=1.6,
            square=1)
...
plt.xlabel('To AA');
plt.ylabel('From AA');
plt.title('Full matrix for pan-TCR weights');

plt.subplot(2, 2, 3)
sns.heatmap((symm_specific/3.1)/blosum,cmap="RdBu",
            # divided the pooled mean of AA matrix So that ratio with blosum=1 
            yticklabels=AA_list,
            xticklabels=AA_list,center=1,vmin=0,vmax=1.6,
            square=1)
...
plt.xlabel('To AA');
plt.ylabel('From AA');
plt.title('Symmetric matrix for TCR-specific weights');

plt.subplot(2, 2, 4)
sns.heatmap((symm_pan/2.2)/blosum,cmap="RdBu",
            # divided the pooled mean of AA matrix So that ratio with blosum=1 
            yticklabels=AA_list,
            xticklabels=AA_list,center=1,vmin=0,vmax=1.6,
            square=1)
...
plt.xlabel('To AA');
plt.ylabel('From AA');
plt.title('Symmetric matrix for pan-TCR weights');

plt.show()
plt.savefig('edfig_7_b-e.pdf', format='pdf')

# Save ratios
aa_ratio_within = (full_specific/2.984)/blosum
aa_ratio_cross = (full_pan/2.279)/blosum

aa_ratio_within.to_csv('aa_ratio_within.csv')
aa_ratio_cross.to_csv('aa_ratio_cross.csv')

aa_within = (full_specific/2.984)
aa_cross = (full_pan/2.279)

aa_within.to_csv('aa_within.csv')
aa_cross.to_csv('aa_cross.csv')
