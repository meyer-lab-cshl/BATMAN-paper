...
## Extended Data Figure 2c: average correlation (over 5 folds) for TCRs
#############################
...
for (AA_distance in 1:70) {
  ...
  for (tcr in 1:66){
    ...
    all_AUC[AA_distance,tcr]<-mean(regression_results$correlations)
  }
}
...
for (tcr in 1:66){
  load(paste("../data/within_tcr_unpooled_outputs_atchley//output_",
                                                  tcr,".rda",sep=""))
  all_AUC[71,tcr]<-mean(correlations)
}
...
p <- p + geom_boxplot(outlier.colour = NA,show.legend = FALSE) +
  geom_jitter(aes(color=as.factor(tcr)), alpha=0.5, width=0.1, height=0, show.legend = FALSE) +
...
ggsave(plot=p, "../figures/extended_data_fig2/correlation_results.pdf",
       width=20, height=6.5)
