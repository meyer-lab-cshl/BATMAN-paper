...
# Subset for length restriction (https://www.nature.com/articles/s42003-024-06380-6)
tcr_pmhc_data = tcr_pmhc_data[tcr_pmhc_data['peptide'].str.len()<=15]


# format for heteroTCR
tcr_pmhc_data = tcr_pmhc_data[['cdr3b','peptide']]
tcr_pmhc_data.columns = ['cdr3', 'peptide']

# Write peptide seqs to output files
tcr_pmhc_data.to_csv('test.tsv',sep='\t', index=False)
