...
################################################################
'''
Function to assign binding category to peptide activity data
'''
def assign_activation_category(df):# input DataFRame
    ...
    if isinstance(df, pd.DataFrame)==False:
        sys.exit("Input is not a Pandas DataFrame")
   
    # Check that input df has relevant columns
    if {'tcr','peptide','peptide_activity'}.issubset(
            set(df.columns))==False:
        sys.exit(''.join(["Input dataframe must have these 3 columns:", 
                 " 'tcr','peptide','peptide_activity'.",
                 "Check spelling and have all columns in lower case."]))
    
    # Check that all peptide activities are numeric and non-negative
    if np.array(df['peptide_activity']).dtype not in ('int','float'):
        sys.exit('peptide_activity must be numeric')
        
    if np.min(np.array(df['peptide_activity']))<0:
        sys.exit('peptide_activity must be non-negative, please scale accordingly') 
        
    # Get unique TCRs in data
    TCR_names, TCR_index = np.unique(df['tcr'], return_inverse=True)
    
    # Normalize peptide activities and indicate peptide activity level
    for TCR in TCR_names:
        activity = df.loc[df.tcr==TCR,'peptide_activity']
        max_activity = np.max(activity)
        df.loc[df.tcr==TCR,
                         'peptide_activity'] = activity/max_activity
        
    peptide_activity = df.peptide_activity
    ...
    df['activation'] = peptide_activity_level.astype(int)
...
def generate_mutant_features(index_peptide, # Single index peptide or list 
    ...
    if isinstance(aa_matrix, str): #Name of AA matrix provided
        ...
        if os.path.isfile(os.path.join(data_directory,filename))==False:
...
def pooled_inference(TCR_index, # TCR index for each peptide
    ...
    # Extract HD of weights
    inferred_weights_sd=abs(np.reshape(inferred_params.iloc[
        weight_index:weight_index+n_tcr*peptide_length,
        2].to_numpy(),newshape=(n_tcr,peptide_length),order='C') - 
    np.reshape(inferred_params.iloc[
        weight_index:weight_index+n_tcr*peptide_length,
        3].to_numpy(),newshape=(n_tcr,peptide_length),order='C'))
    ...
    if np.sum(peptide_bindlevel)==0 and np.sum(mhc_features)==0: #No MHC info available
        ...
        return inferred_weights,inferred_weights_sd,inferred_aa_matrix,inferred_mhc_effects
##############################################################################
...
def train(filename,#Path to file or pandas df with TCR data (see example for format)
    ...
    if mode=='weights_only':
        if consider_mhc_binding: #MHC binding info present
            ...
            inferred_mhc_effects,inferred_weights = pooled_inference_weights_only(TCR_index, 
                                                             peptide_feature, 
        ...
        # Run Inference
        if consider_mhc_binding: #MHC binding info present
            inferred_weights,inferred_weights_sd,inferred_aa_matrix,inferred_mhc_effect = pooled_inference(TCR_index,
                                                peptide_feature, 
                                                peptide_activation_category, 
                                                aa_change_index, 
                                                mode=mode,
    ...
    if consider_mhc_binding==False: #MHC binding info absent
       ...
       # add MHC binding category
        ...
        return inferred_weights,inferred_weights_sd,inferred_aa_matrix,inferred_mhc_effect
